...
beforeAll(async () => {
    ...
    const childComment =  new CommentModel({postId: post.postId, userId: newUser._id.toString(), text: 'childCommentOne', parentCommentId: parentCommentId})
...
describe("GET /api/posts/:postId/comments/:commentId/expand", () => {
    it('should get a comment', async () => {
        const res = await request(app)
            .get(`/api/posts/${post.postId}/comments/${parentCommentId}/expand`)
        ...
        expect(response.message).toBe('Comments expanded successfully.')
    })
    ...
    it('should throw an error for invalid token', async () => {
        ...
        const response = res.body
        console.log('for invalid token', response)
    ...
    it('should throw an error for unknown post id', async () => {
        const res = await request(app)
            .get(`/api/posts/${4}/comments/${parentCommentId}/expand`)
            .set('Authorization', 'Bearer ' + token)
