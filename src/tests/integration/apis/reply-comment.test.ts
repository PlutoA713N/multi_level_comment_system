...
describe("POST /:postId/comments/:commentId/reply", () => {
    it('should create a reply comment', async () => {
        const res = await request(app)
            .post(`/api/posts/${post.postId}/comments/${parentCommentId}/reply`)
        ...
        expect(response.message).toBe('Reply to comment created successfully')
    })
    ...
    it('should throw an error for unknown post id', async () => {
        const res = await request(app)
            .post(`/api/posts/${4}/comments/${parentCommentId}/reply`)
            .set('Authorization', 'Bearer ' + token)
            .send({text: 'Hello world'})
